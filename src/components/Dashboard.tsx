...
const BRAND = {
  ...
  text: "#1A1714",        // dark warm text
  muted: "#8C8579",       // warm gray
  subtle: "#B5AFA6",      // lighter muted
  primary: "#D97757",     // Anthropic terracotta/coral
  primaryHover: "#C26644",
  accent1: "#C96442",     // deeper terracotta
  accent2: "#A47764",     // warm brown
  accent3: "#788C5D",     // sage green
  accent4: "#5B8A9A",     // muted teal
  accent5: "#9B7CB8",     // muted purple
};
...
export default function Dashboard() {
  ...
  const handleExport = async (format: "png" | "jpeg") => {
    ...
    try {
      ...
      const options = {
        ...
        pixelRatio: 2,
        style: {
          // Force computed fonts so they embed in the snapshot
          fontFamily: "inherit",
        },
      };

      const dataUrl = format === "png"
        ? await toPng(dashRef.current, options)
        : await toJpeg(dashRef.current, { ...options, quality: 0.95 });

      const link = document.createElement("a");
      const ext = format === "png" ? "png" : "jpg";
      link.download = `claude_webinar_dashboard_${new Date().toISOString().split("T")[0]}.${ext}`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
  ...
  const teamData = countBy(data, "team_size");
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col" style={{ backgroundColor: BRAND.bg }}>
      ...
      <div className="shrink-0 px-5 pt-3 pb-1 flex items-center justify-end gap-2">
        <button
          onClick={fetchData}
        ...
        <button
          ...
          className="border rounded-lg px-3 py-1.5 text-xs transition-colors hover:bg-white disabled:opacity-40"
          style={{ borderColor: BRAND.border, color: BRAND.muted }}
        >
          {exporting ? "..." : "PNG"}
        </button>
      ...
      {/* Dashboard content — captured for export */}
      <div ref={dashRef} className="flex-1 flex flex-col px-5 pb-4 gap-3 min-h-0" style={{ backgroundColor: BRAND.bg }}>
        {/* Header inside capture area */}
        <div className="shrink-0 flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-xl leading-none" style={{ color: BRAND.text, fontFamily: "var(--font-outfit), 'Segoe UI', Roboto, Helvetica, Arial, sans-serif" }}>Claude Webinar - PreSurvey</h1>
        ...
        {/* Row 2: Business Types (wide) + Language Preference (donut) */}
        {loading ? (
          ...
          <div className="grid grid-cols-3 gap-3 flex-1 min-h-0">
            <div className="col-span-2 bg-white rounded-xl p-4 border flex flex-col"
                 style={{ borderColor: BRAND.border }}>
              <h3 className="text-[10px] uppercase tracking-widest font-semibold mb-2 shrink-0"
        ...
        {/* Row 3: Pain Points + Cities + AI Experience */}
        {loading ? (
          <div className="grid grid-cols-3 gap-3 flex-1 min-h-0">
            <ShimmerChart />
            <ShimmerChart />
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart layout="vertical" data={painData} margin={{ left: 10, right: 10, top: 2, bottom: 2 }}>
                    ...
                    <YAxis
                      ...
                      dataKey="name"
                      width={130}
                      tick={{ fontSize: 9, fill: BRAND.muted }}
                      axisLine={false}
                      tickLine={false}
                    />
                    <Tooltip content={<AnthropicTooltip />} cursor={{ fill: "rgba(217,119,87,0.06)" }} />
        ...
        {/* Bottom Summary Bar */}
        {!loading && (
          <div className="shrink-0 bg-white rounded-xl border py-3 flex items-center justify-center gap-10"
               style={{ borderColor: BRAND.border }}>
            ...
            <div className="text-center">
              <p className="text-lg font-bold leading-none" style={{ color: BRAND.accent3, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace" }}>{todayCount}</p>
              <p className="text-[9px] uppercase tracking-widest mt-0.5" style={{ color: BRAND.subtle }}>Today</p>
            </div>
            {teamData.slice(0, 4).map((t) => (
              <div key={t.name} className="text-center">
                <p className="text-lg font-bold leading-none" style={{ color: BRAND.accent5, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace" }}>{t.count}</p>
                <p className="text-[9px] uppercase tracking-widest mt-0.5" style={{ color: BRAND.subtle }}>Team {t.name}</p>
              </div>
            ))}
          </div>
        )}
